...
def clean_sql(raw_text: str) -> str:
    ...
    text = raw_text.strip()
    if text.startswith("```"):
        # Remove the opening fence (with or without a language tag like ```sql)
        text = text.split("\n", 1)[1] if "\n" in text else text
...
def is_safe_select(sql_text: str) -> bool:
    """Guardrail: only allow read-only SELECT statements."""
...
def ask_question(user_question: str):
    ...
    if not is_safe_select(generated_sql):
        print("BLOCKED: generated statement failed the safety check, refusing to run it.")
        return
...
if __name__ == "__main__":
    ask_question("What are the top 5 products by revenue?")